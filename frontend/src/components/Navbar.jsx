...
export default function Navbar({ scrollToAbout, scrollToContact }) {
    ...
    // useMemo: memoize current user and role path to prevent decoding/recomputing on renders
    const currentUser = useMemo(() => getCurrentUser(), [authenticated]);
    const rolePath = useMemo(() => currentUser?.role?.toLowerCase() || "", [currentUser]);
